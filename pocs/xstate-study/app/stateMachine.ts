...
const LOCKED_DEVICE_ERROR = "Locked device";
const APP_NOT_FOUND_ERROR = "App not installed";
const OPEN_REJECTED_ERROR = "Open app rejected";
...
function makeOpenAppMachine(params: {
  ...
  return setup({
    ...
    actions: {
      assignErrorDeviceNotOnboarded: assign({error: new Error("Device not onboarded")}),
      assignErrorDeviceLocked: assign({error: new Error("Device locked")})
    },
  }).createMachine({
