...
describe("GetFullViewingKeyCommand", () => {
  ...
  describe("getApdu", () => {
    ...
    it("should throw for UFVK when the transparent derivation path is missing", () => {
      const command = new GetFullViewingKeyCommand({
        isContinue: false,
        p2: P2_VK.UFVK,
        derivationPath: orchardPath,
        // Empty path: the type cannot enforce non-emptiness, so the runtime
        // guard must reject it rather than emit the pre-v3.8.0 APDU.
        transparentDerivationPath: "",
      });
      expect(() => command.getApdu()).toThrow(
        /UFVK export requires a transparent derivation path/,
      );
    });
