...
describe("ComputeAssociatedTokenAccountAddress", () => {
  ...
  it("derives the correct ATA and returns the base58 string", async () => {
    // given
    (getAssociatedTokenAddress as Mock).mockResolvedValue(new PublicKey(ATA));
    ...
    const [calledMint, calledOwner, allowOffCurve, tp, atp] = (
      getAssociatedTokenAddress as Mock<
        Mocked<typeof getAssociatedTokenAddress>
      >
    ).mock.calls[0]!;
  ...
  it("propagates errors from getAssociatedTokenAddress", async () => {
    // given
    (getAssociatedTokenAddress as Mock).mockRejectedValue(
