...
describe("SigningOperationsDeviceAction", () => {
  ...
  it("happy path: preview accepted then delayed sign (INS 0x09)", () =>
  ...
  it("protocol fallback (0x6d00): preview unsupported then fallback sign (INS 0x06)", () =>
    new Promise<void>((resolve, reject) => {
  ...
  it("user rejection (0x6985) during preview then error (no fallback)", () =>
    new Promise<void>((resolve, reject) => {
      previewMock.mockResolvedValue(
        CommandResultFactory({
          error: new SolanaAppCommandError({
  ...
  it("generic preview error then fallback sign", () =>
    new Promise<void>((resolve, reject) => {
      previewMock.mockResolvedValue(
        CommandResultFactory({
  ...
  it("fetch blockhash failure then signs the original tx (best-effort, no patch)", () =>
    new Promise<void>((resolve, reject) => {
      previewMock.mockResolvedValue(CommandResultFactory({ data: Nothing }));
  ...
  it("zero-blockhash failure then degrades to a one-shot sign of the original (best-effort)", () =>
    new Promise<void>((resolve, reject) => {
      zeroBlockhashMock.mockRejectedValue(new Error("zeroing failed"));
      signMock.mockResolvedValue(
        CommandResultFactory({ data: Just(exampleSignature) }),
      );
  ...
  it("preview throw (not a user rejection) then degrades to a one-shot sign of the original (best-effort)", () =>
    new Promise<void>((resolve, reject) => {
      previewMock.mockRejectedValue(new Error("preview threw"));
      signMock.mockResolvedValue(
        CommandResultFactory({ data: Just(exampleSignature) }),
      );
  ...
  it("patch failure then signs the original tx delayed (best-effort, matches fetch failure)", () =>
    new Promise<void>((resolve, reject) => {
      previewMock.mockResolvedValue(CommandResultFactory({ data: Nothing }));
      fetchBlockhashMock.mockResolvedValue(exampleBlockhash);
      patchBlockhashMock.mockRejectedValue(new Error("patch failed"));
      delayedSignMock.mockResolvedValue(
        CommandResultFactory({ data: Just(exampleSignature) }),
      );
      ...
      testDeviceActionStates(action, expected, apiMock, {
        onDone: () => {
          // Patch failed, so the original (un-patched) tx is signed delayed.
          expect(delayedSignMock).toHaveBeenCalledWith(
            expect.objectContaining({
              input: expect.objectContaining({
                serializedTransaction: exampleTx,
  ...
  it("delayed sign failure (0x6f11 hash mismatch) then error", () =>
    new Promise<void>((resolve, reject) => {
      previewMock.mockResolvedValue(CommandResultFactory({ data: Nothing }));
      fetchBlockhashMock.mockResolvedValue(exampleBlockhash);
      delayedSignMock.mockResolvedValue(
