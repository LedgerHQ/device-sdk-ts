...
export class DefaultSignerSolana implements SignerSolana {
   ...
   *
   * `{ observable, cancel }` where:
   * - `observable` emits `DeviceActionState<string, GetAddressDAError, IntermediateValue>` updates.
   *   On **Completed**, `output` is the base58-encoded Solana address.
   * - `cancel` aborts the action on the Ledger device.
   *
   * @example
   * ```ts
   ...
   *   next: (state) => {
   *     if (state.status === DeviceActionStatus.Completed) {
   *       console.log("Address:", state.output); // base58 string
   *     }
   *   },
   * });
   * ```
