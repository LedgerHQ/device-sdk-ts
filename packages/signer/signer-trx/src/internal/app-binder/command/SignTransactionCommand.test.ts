...
describe("SignTransactionCommand", () => {
  ...
  describe("getApdu", () => {
    it("should build the APDU with CLA=0xe0, INS=0x04, the given P1 and payload", () => {
      const command = new SignTransactionCommand({ payload, p1: 0x80 });
  ...
  describe("parseResponse", () => {
    it("should return the 65-byte signature on the final frame", () => {
      ...
      const result = new SignTransactionCommand({
        payload,
        p1: 0x10,
    ...
    it("should return an empty signature for an intermediate frame", () => {
      const response = new ApduResponse({
        statusCode: new Uint8Array([0x90, 0x00]),
    ...
    it("should return a TronAppCommandError on a device error status", () => {
      ...
      if (!isSuccessCommandResult(result)) {
        expect((result.error as TronAppCommandError).errorCode).toBe("6a80");
      }
    });
  });
});
