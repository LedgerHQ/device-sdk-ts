...
describe("encodeDerivationPath", () => {
  describe("Given a 5-level derivation path", () => {
    ...
    describe("When the path does not have exactly 5 levels", () => {
      it("Then it should throw for a shorter path instead of zero-padding", () => {
        expect(() => encodeDerivationPath([44, 223, 0])).toThrow(
          "encodeDerivationPath: expected 5 path elements, got 3",
        );
      });

      it("Then it should throw for a longer path", () => {
        expect(() => encodeDerivationPath([44, 223, 0, 0, 0, 0])).toThrow(
          "encodeDerivationPath: expected 5 path elements, got 6",
        );
      });
    });
