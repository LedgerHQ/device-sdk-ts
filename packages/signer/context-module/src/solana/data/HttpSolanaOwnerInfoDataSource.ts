...
export class HttpSolanaOwnerInfoDataSource implements SolanaDataSource {
  ...
  async fetchAddressMetadata(
    ...
    return await axios
      .request<SolanaSPLOwnerInfo>({
        method: "GET",
        url: `${this.config.solanaMetadataService.url}/solana/owner/${tokenAddress}?challenge=${challenge}`,
        headers: {
  ...
  async computeAddressMetadata(
    ...
    return await axios
      .request<SolanaSPLOwnerInfo>({
        method: "GET",
        url: `${this.config.solanaMetadataService.url}/solana/computed-token-account/${address}/${mintAddress}?challenge=${challenge}`,
        headers: {
          [LEDGER_CLIENT_VERSION_HEADER]: `context-module/${PACKAGE.version}`,
