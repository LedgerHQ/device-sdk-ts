...
describe("SolanaTransactionCheckLoader", () => {
  ...
  describe("load — wire format", () => {
    ...
    it("bs58-encodes the bare serialized message as tx.raw", async () => {
      // Per the web3checks OpenAPI spec, `raw` for Solana is the serialized
      // message (not a wrapped transaction with sig placeholders). The device
      // computes sha256(message) to verify the descriptor's TX_HASH.
