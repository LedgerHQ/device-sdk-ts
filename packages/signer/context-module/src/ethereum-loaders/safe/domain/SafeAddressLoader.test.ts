...
} from "@/ethereum-loaders/safe/data/SafeAccountDataSource";
import {
  type SafeAddressContextInput,
  SafeAddressLoader,
} from "@/ethereum-loaders/safe/domain/SafeAddressLoader";
import {
...
describe("SafeAddressLoader", () => {
  ...
  describe("canHandle function", () => {
    ...
    it("should return true when expected types are subset of SAFE and SIGNER", () => {
      expect(
        loader.canHandle(validInput, [ClearSignContextType.ETHEREUM_SAFE]),
      ).toBe(true);
      expect(
        loader.canHandle(validInput, [ClearSignContextType.ETHEREUM_SIGNER]),
      ).toBe(true);
    });
  ...
  describe("load function", () => {
    ...
    it("should work with different device models", async () => {
      ...
      expect(result[0]?.type).toBe(ClearSignContextType.ETHEREUM_SAFE);
      expect(result[1]?.type).toBe(ClearSignContextType.ETHEREUM_SIGNER);
    });
