...
import { type HttpTokenDataSource } from "@/ethereum-loaders/token/data/HttpTokenDataSource";
import {
  UNISWAP_EXECUTE_SELECTOR,
  UniswapSupportedCommand,
} from "@/ethereum-loaders/uniswap/constants/uniswap";
import { type CommandDecoderDataSource } from "@/ethereum-loaders/uniswap/data/CommandDecoderDataSource";
...
describe("UniswapContextLoader", () => {
  ...
  describe("canHandle function", () => {
    ...
    it.each([
      ...
      expect(
        loader.canHandle(input, [ClearSignContextType.ETHEREUM_TOKEN]),
      ).toBe(false);
    });
  });
