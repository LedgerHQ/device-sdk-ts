...
import { KeyId } from "@/loaders/chain-agnostic/pki/model/KeyId";
...
import type { TypedDataDataSource } from "@/loaders/ethereum/typed-data/data/TypedDataDataSource";
...
type ResolvedProxy = {
  ...
  context?: ClearSignContextSuccess<ClearSignContextType.ETHEREUM_PROXY_INFO>;
};
...
export class DefaultTypedDataContextLoader implements TypedDataContextLoader {
  ...
  private extractCalldatas(
    ...
    for (const calldataIndex in calldatasInfos) {
      ...
      const calldataFiltersForIndex = calldataFilters[calldataIndex];
      if (!calldataFiltersForIndex) {
        continue;
      }

      // Get data
      const data = this.extractHexaString(
        calldataFiltersForIndex.find(
          (filter) => filter.type === "calldata-value",
        ),
        typedData,
        "0x",
      );
      ...
      const selector = this.extractHexaString(
        calldataFiltersForIndex.find(
          (filter) => filter.type === "calldata-selector",
        ),
        typedData,
        data.slice(0, 10),
      );
      ...
      const to = this.extractAddress(
        calldataFiltersForIndex.find(
          (filter) => filter.type === "calldata-callee",
        ),
        typedData,
        infos.calleeFlag,
      );
      ...
      const from = this.extractAddress(
        calldataFiltersForIndex.find(
          (filter) => filter.type === "calldata-spender",
        ),
        typedData,
        infos.spenderFlag,
      );
      ...
      const value = this.extractBigint(
        calldataFiltersForIndex.find(
          (filter) => filter.type === "calldata-amount",
        ),
        typedData,
        undefined,
      );
      ...
      const chainIdBigint = this.extractBigint(
        calldataFiltersForIndex.find(
          (filter) => filter.type === "calldata-chain-id",
        ),
        typedData,
        undefined,
      );
