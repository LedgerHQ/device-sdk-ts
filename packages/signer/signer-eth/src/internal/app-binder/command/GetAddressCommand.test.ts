...
// No chainId in APDU when checkOnDevice is false (data length 21)
const GET_ADDRESS_APDU = Uint8Array.from([
  0xe0, 0x02, 0x00, 0x00, 0x15, 0x05, 0x80, 0x00, 0x00, 0x2c, 0x80, 0x00, 0x00,
  0x3c, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
]);

// chainId 1 (default) when checkOnDevice is true (data length 29)
...
// No chainId when checkOnDevice is false
const GET_ADDRESS_APDU_WITH_RETURN_CHAIN_CODE = Uint8Array.from([
  0xe0, 0x02, 0x00, 0x01, 0x15, 0x05, 0x80, 0x00, 0x00, 0x2c, 0x80, 0x00, 0x00,
  0x3c, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
]);
...
// Chain ID 137 (Polygon) when checkOnDevice is true (p1=0x01, data includes 8-byte chainId)
...
// Chain ID 56 (BSC) when checkOnDevice is true
...
describe("GetAddressCommand", () => {
  ...
  describe("getApdu", () => {
    ...
    it("should default chainId to 1 (Ethereum mainnet) when checkOnDevice is true and chainId is omitted", () => {
      command = new GetAddressCommand({
        ...defaultArgs,
        checkOnDevice: true,
      });
    ...
    it("should send chainId silently (checkOnDevice=false) when explicitly provided — fixture parity with playground register_signer_controlled_ledger_account ex2", () => {
      ...
      const apdu = command.getApdu();
      expect(apdu.getRawApdu()).toStrictEqual(SILENT_WITH_CHAIN_ID);
    });
