...
import {
  RegisterWalletAddressCommand,
  type RegisterWalletAddressCommandArgs,
} from "@internal/app-binder/command/RegisterWalletAddressCommand";
...
const EXPECTED_RAW_APDU = Uint8Array.from(
  [
    0xe1, 0x02, 0x00, 0x01, 0x50, 0x02, 0x0c, 0x43, 0x6f, 0x6c, 0x64, 0x20,
    0x73, 0x74, 0x6f, 0x72, 0x61, 0x67, 0x65, 0x1e,
  ]
...
describe("RegisterWalletAddressCommand", () => {
  let commandArgs: RegisterWalletAddressCommandArgs;
  beforeAll(() => {
    commandArgs = {
      policyName: "Cold storage",
      policyNameLength: "Cold storage".length,
      policyDescriptorTemplateLength: "sh(sortedmulti(2,@0/**,@1/**))".length,
      policyDescriptorTemplateHash: POLICY_HASH_DATA,
      policyKeysLength: 2,
      policyKeysMerkleTreeRoot: KEYS_HASH_DATA,
    };
  });
  describe("getApdu", () => {
    it("should send the correct APDU", () => {
      // when
      const command = new RegisterWalletAddressCommand(commandArgs);
