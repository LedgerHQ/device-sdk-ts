...
export class AxiosManagerApiDataSource implements ManagerApiDataSource {
  ...
  getMcuList(): EitherAsync<HttpFetchApiError, McuFirmware[]> {
    return EitherAsync(() =>
      axios.get<McuVersionDto[]>(`${this._managerApiBaseUrl}/mcu_versions`, {}),
    )
      .map((res) => res.data)
  ...
  ): EitherAsync<HttpFetchApiError, LanguagePackage[]> {
    const { deviceId, currentFinalFirmwareId } = params;
    return EitherAsync(() =>
      axios.get<LanguagePackageVersionDto[]>(
  ...
  private mapLatestFirmwareDto(
    ...
    return EitherAsync<Error, FirmwareOsuVersionDto>(async () => {
      if (
  ...
  private mapLanguagesDtoToLanguages(
    ...
    return EitherAsync(async () => {
      return languages.map((language) => {
        return this.mapLanguageDto(language);
      });
    });
  }
