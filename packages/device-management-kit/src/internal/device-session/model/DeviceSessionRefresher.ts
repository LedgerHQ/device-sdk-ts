...
import { DEVICE_SESSION_REFRESHER_MINIMUM_POLLING_INTERVAL } from "@internal/device-session/data/DeviceSessionRefresherConst";
...
export class DeviceSessionRefresher {
  ...
  start() {
    ...
    this._subscription = iif(
      ...
      refreshObservable.pipe(take(1)), // if _isRefresherDisabled is true, apply take(1) (run only once to get the initial state)
