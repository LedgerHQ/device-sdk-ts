...
import {
  ...
  UploadBackgroundImageChunkCommandError,
} from "./UploadBackgroundImageChunkCommand";
  ...
  describe("parseResponse", () => {
    ...
    it("should return error on create not called (5106)", () => {
      ...
      if (!isSuccessCommandResult(result)) {
        ...
        expect(
          (result.error as UploadBackgroundImageChunkCommandError).message,
        ).toBe("Invalid state, create background image has not been called");
      }
    });
    ...
    it("should return error on invalid chunk offset/length (680b)", () => {
      ...
      if (!isSuccessCommandResult(result)) {
        ...
        expect(
          (result.error as UploadBackgroundImageChunkCommandError).message,
        ).toBe("Invalid chunk offset or length");
      }
    });
