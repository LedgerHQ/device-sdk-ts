...
import { SignMessageParams } from "@near-wallet-selector/core/src/lib/wallet/wallet.types";
...
const Ledger: WalletBehaviourFactory<HardwareWallet> = async ({
  ...
  const signer: Signer = {
    ...
    signMessage: async (message, accountId) => {
      ...
      const { nonce, signerId, receiverId, actions, blockHash } =
        nearAPI.transactions.Transaction.decode(message);

      const signature = await _state.client.signTransaction({
        nonce,
        signerId,
        receiverId,
        actions,
  ...
  return {
    async signIn({ accounts }) {
      ...
      console.log("return", getAccounts());
    ...
    async getPublicKey(derivationPath: string) {
      ...
      const ret = await _state.client.getPublicKey({ derivationPath });
    ...
    async signMessage(params: SignMessageParams) {
      ...
      const ledgerActiveAccount = _state.accounts.find(
        (a) => a.accountId === account.accountId,
      );
      if (!ledgerActiveAccount) {
        throw new Error("No ledger account");
      }
      ...
      const signature = await _state.client.signMessage({
        ...params,
        derivationPath: ledgerActiveAccount.derivationPath,
      });
      return {
        accountId: ledgerActiveAccount.accountId,
        publicKey: ledgerActiveAccount.publicKey,
