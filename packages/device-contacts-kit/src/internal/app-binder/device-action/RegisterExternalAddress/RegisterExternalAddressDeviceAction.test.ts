...
describe("RegisterExternalAddressDeviceAction", () => {
  ...
  it("open-app path, new contact group: OpenApp -> VersionGuard -> RegisterIdentity -> Completed", () =>
    new Promise<void>((resolve, reject) => {
      ...
      const expected = [
        ...
        // WaitForAppAndVersion phase: entry reset, then the child's silent read
        // re-emits None through onSnapshot propagation.
        {
          intermediateValue: {
            requiredUserInteraction: UserInteractionRequired.None,
          },
  ...
  it("version guard rejects on skip-open-app path without opening the app", () =>
    new Promise<void>((resolve, reject) => {
      ...
      const expected = [
        // WaitForAppAndVersion phase (entry reset + child read) runs before the
        // guard; both reads emit None.
        {
          intermediateValue: {
            requiredUserInteraction: UserInteractionRequired.None,
          },
  ...
  it("evaluates support from the fresh WaitForAppAndVersion result, not stale session state", () =>
    new Promise<void>((resolve, reject) => {
      ...
      const expected = [
        // WaitForAppAndVersion phase: entry reset + child read, both None.
        {
          intermediateValue: {
            requiredUserInteraction: UserInteractionRequired.None,
          },
  ...
  it("surfaces a WaitForAppAndVersion failure as the device action error", () =>
    new Promise<void>((resolve, reject) => {
      ...
      const expected = [
        // WaitForAppAndVersion phase: entry reset + child read, both None,
        // before the child DA surfaces its failure.
        {
          intermediateValue: {
            requiredUserInteraction: UserInteractionRequired.None,
          },
  ...
  it("propagates the UnlockDevice interaction emitted by WaitForAppAndVersion", () =>
    new Promise<void>((resolve, reject) => {
      setupOpenAppDAMock();
      // Locked device: the child WaitForAppAndVersion DA requests an unlock
      // before it can read the app/version. The parent must surface it so a
      // consuming client can show a "please unlock your device" prompt.
      ...
      const expected = [
        // Entry reset before the child DA runs.
        {
          intermediateValue: {
            requiredUserInteraction: UserInteractionRequired.None,
          },
        ...
        // Propagated from the child DA: the device needs to be unlocked.
        {
          intermediateValue: {
