...
export const SdkProvider: React.FC<PropsWithChildren> = ({ children }) => {
  ...
  const previousMockServerEnabled =
    previousTransport === BuiltinTransports.MOCK_SERVER;
  ...
  useEffect(() => {
    if (mockServerEnabled) {
      setState(({ logsExporter }) => {
        return {
          sdk: buildMockSdk(mockServerUrl, logsExporter),
          logsExporter,
        };
      });
    } else if (previousMockServerEnabled) {
      setState(({ logsExporter }) => {
        return {
  ...
  }, [mockServerUrl, mockServerEnabled, previousMockServerEnabled]);
