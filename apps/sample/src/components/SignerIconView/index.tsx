...
import { useSignerIcon } from "@/providers/SignerIconProvider";

export const SignerIconView: React.FC<{ sessionId: string }> = ({
  sessionId,
}) => {
  const dmk = useDmk();
  const signer = useSignerIcon();
  ...
  return (
    <DeviceActionsList title="Signer Icon" deviceActions={deviceActions} />
  );
};
