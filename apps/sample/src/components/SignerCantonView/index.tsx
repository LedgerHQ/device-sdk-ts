...
import { useSignerCanton } from "@/providers/SignerCantonProvider";

export const SignerCantonView: React.FC<{ sessionId: string }> = ({
  sessionId,
}) => {
  const dmk = useDmk();
  const signer = useSignerCanton();
  ...
  return (
    <DeviceActionsList title="Signer Canton" deviceActions={deviceActions} />
  );
};
