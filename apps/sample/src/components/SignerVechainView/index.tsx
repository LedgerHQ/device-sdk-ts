...
import { useSignerVechain } from "@/providers/SignerVechainProvider";

export const SignerVechainView: React.FC<{ sessionId: string }> = ({
  sessionId,
}) => {
  const dmk = useDmk();
  const signer = useSignerVechain();
  ...
  return (
    <DeviceActionsList title="Signer Vechain" deviceActions={deviceActions} />
  );
};
