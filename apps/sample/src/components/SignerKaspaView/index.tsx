...
import { useSignerKaspa } from "@/providers/SignerKaspaProvider";

export const SignerKaspaView: React.FC<{ sessionId: string }> = ({
  sessionId,
}) => {
  const dmk = useDmk();
  const signer = useSignerKaspa();
  ...
  return (
    <DeviceActionsList title="Signer Kaspa" deviceActions={deviceActions} />
  );
};
