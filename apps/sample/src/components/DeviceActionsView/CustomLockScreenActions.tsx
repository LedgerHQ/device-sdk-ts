...
export const CustomLockScreenActions: React.FC<{ sessionId: string }> = ({
  sessionId,
}) => {
  const dmk = useDmk();
  ...
  const deviceActions: DeviceActionProps<any, any, any, any>[] = useMemo(() => {
    ...
    return [
      // Upload CLS from raw hex data
      {
        title: "Upload Custom Lock Screen (Raw Hex)",
        description:
        ...
        executeDeviceAction: ({ imageDataHex, unlockTimeout }, inspect) => {
      ...
      // Upload CLS from picture file
      {
        title: "Upload Custom Lock Screen (Picture)",
        description:
      ...
      // Fetch CLS
      {
        title: "Fetch Custom Lock Screen",
        description:
        ...
        { backupHash: string; allowedEmpty: boolean; unlockTimeout: number },
      ...
      // Remove CLS
      {
        title: "Remove Custom Lock Screen",
        description:
  ...
  }, [deviceModelId, dmk, isSupported, sessionId]);
    ...
    return (
      <Flex flexDirection="column" p={4}>
        ...
        <Text variant="small" color="neutral.c70" mt={2}>
          Supported devices: Ledger Stax, Ledger Flex, Ledger Apex
        </Text>
      </Flex>
    );
  }
