...
import { useSignerTezos } from "@/providers/SignerTezosProvider";

export const SignerTezosView: React.FC<{ sessionId: string }> = ({
  sessionId,
}) => {
  const dmk = useDmk();
  const signer = useSignerTezos();
  ...
  return (
    <DeviceActionsList title="Signer Tezos" deviceActions={deviceActions} />
  );
};
