...
import { useSignerHelium } from "@/providers/SignerHeliumProvider";

export const SignerHeliumView: React.FC<{ sessionId: string }> = ({
  sessionId,
}) => {
  const dmk = useDmk();
  const signer = useSignerHelium();
  ...
  return (
    <DeviceActionsList title="Signer Helium" deviceActions={deviceActions} />
  );
};
