...
import { useSignerAptos } from "@/providers/SignerAptosProvider";

export const SignerAptosView: React.FC<{ sessionId: string }> = ({
  sessionId,
}) => {
  const dmk = useDmk();
  const signer = useSignerAptos();
  ...
  return (
    <DeviceActionsList title="Signer Aptos" deviceActions={deviceActions} />
  );
};
