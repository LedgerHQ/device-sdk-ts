...
test("device command: close bitcoin app", async ({ page }) => {
  ...
  // Execute the "Close app" command using `getByTestId`
  await page.getByTestId("CTA_command-Close app").click();

  // Send the close command and verify the response using `getByTestId`
  await page.getByTestId("CTA_send-command").click();
  await expect(page.getByTestId("box_device-commands-responses")).toContainText(
    '"status": "SUCCESS"',
  );
