...
- Retrieving the Cosmos address using a given derivation path
...
This method allows users to retrieve the Cosmos address based on a given `derivationPath` and `hrp` (Human-Readable Part).
...
- `options`
  ...
  - `skipOpenApp`: An optional boolean indicating whether to skip opening the Cosmos app automatically (`true`) or not (`false`).
...
This method allows users to sign a Cosmos transaction.
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"44'/118'/0'/0/0"`)
  - The derivation path used for the Cosmos transaction. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
