...
The `SignerCosmosBuilder.build()` method will return a `SignerCosmos` instance that exposes 4 dedicated methods, each of which calls an independent use case. Each use case will return an object that contains an observable and a method called `cancel`.
...
This method allows users to retrieve the Cosmos address based on a given `derivationPath`.

```typescript
const { observable, cancel } = signerCosmos.getAddress(derivationPath, options);
```
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/118'/0'/0/0"`)
...
```typescript
const { observable, cancel } = signerCosmos.signMessage(
  derivationPath,
  message,
);
```
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/118'/0'/0/0"`)
  - The derivation path used for the Cosmos message. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
