...
- Signing a hyperliquid transaction
...
4. [Use Cases](#-use-cases)
   - [Sign Transaction](#use-case-1-sign-transaction)
...
### Use Case 1: Sign Transaction

This method allows users to sign a hyperliquid transaction.

```typescript
const { observable, cancel } = signerHyperliquid.signTransaction(
  derivationPath,
  transaction,
  options,
);
...
```typescript
const { observable, cancel } = signerHyperliquid.signMessage(
  derivationPath,
  message,
);
```
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/0'/0'/0/0"`)
  - The derivation path used for the hyperliquid message. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
