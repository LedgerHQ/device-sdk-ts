...
This module provides the implementation of the Ledger Tezos signer of the Device Management Kit. It enables interaction with the Tezos application on a Ledger device including:

- Retrieving the Tezos address using a given derivation path
- Signing a Tezos transaction
...
To install the `device-signer-kit-tezos` package, run the following command:

```sh
npm install @ledgerhq/device-signer-kit-tezos
```

## 🔹 Initialisation

To initialise a Tezos signer instance, you need a Ledger Device Management Kit instance and the ID of the session of the connected device. Use the `SignerTezosBuilder`:

```typescript
const signerTezos = new SignerTezosBuilder({ dmk, sessionId }).build();
```

## 🔹 Use Cases

The `SignerTezosBuilder.build()` method will return a `SignerTezos` instance that exposes 4 dedicated methods, each of which calls an independent use case. Each use case will return an object that contains an observable and a method called `cancel`.
...
```typescript
const { observable, cancel } = signerTezos.getAppConfig();
```
...
This method allows users to retrieve the Tezos address based on a given `derivationPath`.

```typescript
const { observable, cancel } = signerTezos.getAddress(derivationPath, options);
```
...
- `derivationPath`
  ...
  - The derivation path used for the Tezos address. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
- `options`
  ...
  - `skipOpenApp`: An optional boolean indicating whether to skip opening the Tezos app automatically (`true`) or not (`false`).
...
This method allows users to sign a Tezos transaction.

```typescript
const { observable, cancel } = signerTezos.signTransaction(
  derivationPath,
  transaction,
  options,
);
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/0'/0'/0/0"`)
  - The derivation path used for the Tezos transaction. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
```typescript
const { observable, cancel } = signerTezos.signMessage(
  derivationPath,
  message,
);
```
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/0'/0'/0/0"`)
  - The derivation path used for the Tezos message. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
We encourage you to explore the Tezos Signer by trying it out in our online [sample application](https://app.devicesdk.ledger-test.com/). Experience how it works and see its capabilities in action. Of course, you will need a Ledger device connected.
