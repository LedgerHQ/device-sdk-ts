...
- Retrieving the Concordium address using a given derivation path
- Signing a Concordium transaction
...
```typescript
const signerConcordium = new SignerConcordiumBuilder({ dmk, sessionId }).build();
```
...
```typescript
const { observable, cancel } = signerConcordium.getAppConfig();
```
...
This method allows users to retrieve the Concordium address based on a given `derivationPath`.

```typescript
const { observable, cancel } = signerConcordium.getAddress(derivationPath, options);
```
...
- `derivationPath`
  ...
  - The derivation path used for the Concordium address. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
- `options`
  ...
  - `skipOpenApp`: An optional boolean indicating whether to skip opening the Concordium app automatically (`true`) or not (`false`).
...
This method allows users to sign a Concordium transaction.
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/0'/0'/0/0"`)
  - The derivation path used for the Concordium transaction. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
```typescript
const { observable, cancel } = signerConcordium.signMessage(
  derivationPath,
  message,
);
```
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/0'/0'/0/0"`)
  - The derivation path used for the Concordium message. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
We encourage you to explore the Concordium Signer by trying it out in our online [sample application](https://app.devicesdk.ledger-test.com/). Experience how it works and see its capabilities in action. Of course, you will need a Ledger device connected.
