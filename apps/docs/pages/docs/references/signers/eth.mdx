...
- `options`
  ...
  - `chainId`: An optional number for the Ethereum chain ID (e.g., `1` for mainnet, `137` for Polygon). **Only used when `checkOnDevice` is `true`.** When enabled, it is sent to the device for address verification and is used to load dynamic network context (e.g. network name and icon). When `checkOnDevice` is `false`, `chainId` is ignored.
