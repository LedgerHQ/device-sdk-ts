...
This module provides the implementation of the Ledger Vechain signer of the Device Management Kit. It enables interaction with the Vechain application on a Ledger device including:

- Retrieving the Vechain address using a given derivation path
...
To install the `device-signer-kit-vechain` package, run the following command:

```sh
npm install @ledgerhq/device-signer-kit-vechain
```

## 🔹 Initialisation

To initialise a Vechain signer instance, you need a Ledger Device Management Kit instance and the ID of the session of the connected device. Use the `SignerVechainBuilder`:

```typescript
const signerVechain = new SignerVechainBuilder({ dmk, sessionId }).build();
```

## 🔹 Use Cases

The `SignerVechainBuilder.build()` method will return a `SignerVechain` instance that exposes 4 dedicated methods, each of which calls an independent use case. Each use case will return an object that contains an observable and a method called `cancel`.
...
```typescript
const { observable, cancel } = signerVechain.getAppConfig();
```
...
This method allows users to retrieve the Vechain address based on a given `derivationPath`.

```typescript
const { observable, cancel } = signerVechain.getAddress(derivationPath, options);
```
...
- `derivationPath`
  ...
  - The derivation path used for the Vechain address. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
- `options`
  ...
  - `skipOpenApp`: An optional boolean indicating whether to skip opening the Vechain app automatically (`true`) or not (`false`).
...
This method allows users to sign a Vechain transaction.

```typescript
const { observable, cancel } = signerVechain.signTransaction(
  derivationPath,
  transaction,
  options,
);
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/0'/0'/0/0"`)
  - The derivation path used for the Vechain transaction. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
```typescript
const { observable, cancel } = signerVechain.signMessage(
  derivationPath,
  message,
);
```
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/0'/0'/0/0"`)
  - The derivation path used for the Vechain message. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
We encourage you to explore the Vechain Signer by trying it out in our online [sample application](https://app.devicesdk.ledger-test.com/). Experience how it works and see its capabilities in action. Of course, you will need a Ledger device connected.
