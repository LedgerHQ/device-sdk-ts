...
This module provides the implementation of the Ledger Helium signer of the Device Management Kit. It enables interaction with the Helium application on a Ledger device including:

- Retrieving the Helium address using a given derivation path
- Signing a Helium transaction
...
To install the `device-signer-kit-helium` package, run the following command:

```sh
npm install @ledgerhq/device-signer-kit-helium
```

## 🔹 Initialisation

To initialise a Helium signer instance, you need a Ledger Device Management Kit instance and the ID of the session of the connected device. Use the `SignerHeliumBuilder`:

```typescript
const signerHelium = new SignerHeliumBuilder({ dmk, sessionId }).build();
```

## 🔹 Use Cases

The `SignerHeliumBuilder.build()` method will return a `SignerHelium` instance that exposes 4 dedicated methods, each of which calls an independent use case. Each use case will return an object that contains an observable and a method called `cancel`.
...
```typescript
const { observable, cancel } = signerHelium.getAppConfig();
```
...
This method allows users to retrieve the Helium address based on a given `derivationPath`.

```typescript
const { observable, cancel } = signerHelium.getAddress(derivationPath, options);
```
...
- `derivationPath`
  ...
  - The derivation path used for the Helium address. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
- `options`
  ...
  - `skipOpenApp`: An optional boolean indicating whether to skip opening the Helium app automatically (`true`) or not (`false`).
...
This method allows users to sign a Helium transaction.

```typescript
const { observable, cancel } = signerHelium.signTransaction(
  derivationPath,
  transaction,
  options,
);
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/0'/0'/0/0"`)
  - The derivation path used for the Helium transaction. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
```typescript
const { observable, cancel } = signerHelium.signMessage(
  derivationPath,
  message,
);
```
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/0'/0'/0/0"`)
  - The derivation path used for the Helium message. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
We encourage you to explore the Helium Signer by trying it out in our online [sample application](https://app.devicesdk.ledger-test.com/). Experience how it works and see its capabilities in action. Of course, you will need a Ledger device connected.
