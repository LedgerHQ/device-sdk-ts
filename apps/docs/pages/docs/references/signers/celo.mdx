...
This module provides the implementation of the Ledger Celo signer of the Device Management Kit. It enables interaction with the Celo application on a Ledger device including:

- Retrieving the Celo address using a given derivation path
- Signing a Celo transaction
...
To install the `device-signer-kit-celo` package, run the following command:

```sh
npm install @ledgerhq/device-signer-kit-celo
```

## 🔹 Initialisation

To initialise a Celo signer instance, you need a Ledger Device Management Kit instance and the ID of the session of the connected device. Use the `SignerCeloBuilder`:

```typescript
const signerCelo = new SignerCeloBuilder({ dmk, sessionId }).build();
```

## 🔹 Use Cases

The `SignerCeloBuilder.build()` method will return a `SignerCelo` instance that exposes 4 dedicated methods, each of which calls an independent use case. Each use case will return an object that contains an observable and a method called `cancel`.
...
```typescript
const { observable, cancel } = signerCelo.getAppConfig();
```
...
This method allows users to retrieve the Celo address based on a given `derivationPath`.

```typescript
const { observable, cancel } = signerCelo.getAddress(derivationPath, options);
```
...
- `derivationPath`
  ...
  - The derivation path used for the Celo address. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
- `options`
  ...
  - `skipOpenApp`: An optional boolean indicating whether to skip opening the Celo app automatically (`true`) or not (`false`).
...
This method allows users to sign a Celo transaction.

```typescript
const { observable, cancel } = signerCelo.signTransaction(
  derivationPath,
  transaction,
  options,
);
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/0'/0'/0/0"`)
  - The derivation path used for the Celo transaction. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
```typescript
const { observable, cancel } = signerCelo.signMessage(
  derivationPath,
  message,
);
```
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/0'/0'/0/0"`)
  - The derivation path used for the Celo message. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
We encourage you to explore the Celo Signer by trying it out in our online [sample application](https://app.devicesdk.ledger-test.com/). Experience how it works and see its capabilities in action. Of course, you will need a Ledger device connected.
