...
This module provides the implementation of the Ledger Aptos signer of the Device Management Kit. It enables interaction with the Aptos application on a Ledger device including:

- Retrieving the Aptos address using a given derivation path
- Signing a Aptos transaction
...
To install the `device-signer-kit-aptos` package, run the following command:

```sh
npm install @ledgerhq/device-signer-kit-aptos
```

## 🔹 Initialisation

To initialise a Aptos signer instance, you need a Ledger Device Management Kit instance and the ID of the session of the connected device. Use the `SignerAptosBuilder`:

```typescript
const signerAptos = new SignerAptosBuilder({ dmk, sessionId }).build();
```

## 🔹 Use Cases

The `SignerAptosBuilder.build()` method will return a `SignerAptos` instance that exposes 4 dedicated methods, each of which calls an independent use case. Each use case will return an object that contains an observable and a method called `cancel`.
...
```typescript
const { observable, cancel } = signerAptos.getAppConfig();
```
...
This method allows users to retrieve the Aptos address based on a given `derivationPath`.

```typescript
const { observable, cancel } = signerAptos.getAddress(derivationPath, options);
```
...
- `derivationPath`
  ...
  - The derivation path used for the Aptos address. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
- `options`
  ...
  - `skipOpenApp`: An optional boolean indicating whether to skip opening the Aptos app automatically (`true`) or not (`false`).
...
This method allows users to sign a Aptos transaction.

```typescript
const { observable, cancel } = signerAptos.signTransaction(
  derivationPath,
  transaction,
  options,
);
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/0'/0'/0/0"`)
  - The derivation path used for the Aptos transaction. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
```typescript
const { observable, cancel } = signerAptos.signMessage(
  derivationPath,
  message,
);
```
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/0'/0'/0/0"`)
  - The derivation path used for the Aptos message. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
We encourage you to explore the Aptos Signer by trying it out in our online [sample application](https://app.devicesdk.ledger-test.com/). Experience how it works and see its capabilities in action. Of course, you will need a Ledger device connected.
