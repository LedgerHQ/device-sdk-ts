...
This module provides the implementation of the Ledger Stellar signer of the Device Management Kit. It enables interaction with the Stellar application on a Ledger device including:

- Retrieving the Stellar address using a given derivation path
...
To install the `device-signer-kit-stellar` package, run the following command:

```sh
npm install @ledgerhq/device-signer-kit-stellar
```

## 🔹 Initialisation

To initialise a Stellar signer instance, you need a Ledger Device Management Kit instance and the ID of the session of the connected device. Use the `SignerStellarBuilder`:

```typescript
const signerStellar = new SignerStellarBuilder({ dmk, sessionId }).build();
```

## 🔹 Use Cases

The `SignerStellarBuilder.build()` method will return a `SignerStellar` instance that exposes 4 dedicated methods, each of which calls an independent use case. Each use case will return an object that contains an observable and a method called `cancel`.
...
```typescript
const { observable, cancel } = signerStellar.getAppConfig();
```
...
This method allows users to retrieve the Stellar address based on a given `derivationPath`.

```typescript
const { observable, cancel } = signerStellar.getAddress(derivationPath, options);
```
...
- `derivationPath`
  ...
  - The derivation path used for the Stellar address. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
- `options`
  ...
  - `skipOpenApp`: An optional boolean indicating whether to skip opening the Stellar app automatically (`true`) or not (`false`).
...
This method allows users to sign a Stellar transaction.

```typescript
const { observable, cancel } = signerStellar.signTransaction(
  derivationPath,
  transaction,
  options,
);
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/0'/0'/0/0"`)
  - The derivation path used for the Stellar transaction. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
```typescript
const { observable, cancel } = signerStellar.signMessage(
  derivationPath,
  message,
);
```
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/0'/0'/0/0"`)
  - The derivation path used for the Stellar message. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
We encourage you to explore the Stellar Signer by trying it out in our online [sample application](https://app.devicesdk.ledger-test.com/). Experience how it works and see its capabilities in action. Of course, you will need a Ledger device connected.
