...
The `SignerPolkadotBuilder.build()` method will return a `SignerPolkadot` instance that exposes 4 dedicated methods, each of which calls an independent use case. Each use case will return an object that contains an observable and a method called `cancel`.
...
```typescript
const { observable, cancel } = signerPolkadot.getAppConfig();
```
...
```typescript
const { observable, cancel } = signerPolkadot.getAddress(derivationPath, options);
```
...
```typescript
const { observable, cancel } = signerPolkadot.signMessage(
  derivationPath,
  message,
);
```
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/0'/0'/0/0"`)
  - The derivation path used for the Polkadot message. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
