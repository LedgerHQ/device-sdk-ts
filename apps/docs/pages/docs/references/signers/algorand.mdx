...
This module provides the implementation of the Ledger Algorand signer of the Device Management Kit. It enables interaction with the Algorand application on a Ledger device including:

- Retrieving the Algorand address using a given derivation path
- Signing a Algorand transaction
...
To install the `device-signer-kit-algorand` package, run the following command:

```sh
npm install @ledgerhq/device-signer-kit-algorand
```

## 🔹 Initialisation

To initialise a Algorand signer instance, you need a Ledger Device Management Kit instance and the ID of the session of the connected device. Use the `SignerAlgorandBuilder`:

```typescript
const signerAlgorand = new SignerAlgorandBuilder({ dmk, sessionId }).build();
```

## 🔹 Use Cases

The `SignerAlgorandBuilder.build()` method will return a `SignerAlgorand` instance that exposes 4 dedicated methods, each of which calls an independent use case. Each use case will return an object that contains an observable and a method called `cancel`.
...
```typescript
const { observable, cancel } = signerAlgorand.getAppConfig();
```
...
This method allows users to retrieve the Algorand address based on a given `derivationPath`.

```typescript
const { observable, cancel } = signerAlgorand.getAddress(derivationPath, options);
```
...
- `derivationPath`
  ...
  - The derivation path used for the Algorand address. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
- `options`
  ...
  - `skipOpenApp`: An optional boolean indicating whether to skip opening the Algorand app automatically (`true`) or not (`false`).
...
This method allows users to sign a Algorand transaction.

```typescript
const { observable, cancel } = signerAlgorand.signTransaction(
  derivationPath,
  transaction,
  options,
);
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/0'/0'/0/0"`)
  - The derivation path used for the Algorand transaction. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
```typescript
const { observable, cancel } = signerAlgorand.signMessage(
  derivationPath,
  message,
);
```
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/0'/0'/0/0"`)
  - The derivation path used for the Algorand message. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
We encourage you to explore the Algorand Signer by trying it out in our online [sample application](https://app.devicesdk.ledger-test.com/). Experience how it works and see its capabilities in action. Of course, you will need a Ledger device connected.
