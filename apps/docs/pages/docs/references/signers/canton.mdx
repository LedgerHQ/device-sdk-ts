...
This module provides the implementation of the Ledger Canton signer of the Device Management Kit. It enables interaction with the Canton application on a Ledger device including:

- Retrieving the Canton address using a given derivation path
- Signing a Canton transaction
...
To install the `device-signer-kit-canton` package, run the following command:
...
To initialise a Canton signer instance, you need a Ledger Device Management Kit instance and the ID of the session of the connected device. Use the `SignerCantonBuilder`:

```typescript
const signerCanton = new SignerCantonBuilder({ dmk, sessionId }).build();
```

## 🔹 Use Cases

The `SignerCantonBuilder.build()` method will return a `SignerCanton` instance that exposes 4 dedicated methods, each of which calls an independent use case. Each use case will return an object that contains an observable and a method called `cancel`.
...
```typescript
const { observable, cancel } = signerCanton.getAppConfig();
```
...
This method allows users to retrieve the Canton address based on a given `derivationPath`.

```typescript
const { observable, cancel } = signerCanton.getAddress(derivationPath, options);
```
...
- `derivationPath`
  ...
  - The derivation path used for the Canton address. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
- `options`
  ...
  - `skipOpenApp`: An optional boolean indicating whether to skip opening the Canton app automatically (`true`) or not (`false`).
...
This method allows users to sign a Canton transaction.

```typescript
const { observable, cancel } = signerCanton.signTransaction(
  derivationPath,
  transaction,
  options,
);
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/0'/0'/0/0"`)
  - The derivation path used for the Canton transaction. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
```typescript
const { observable, cancel } = signerCanton.signMessage(
  derivationPath,
  message,
);
```
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/0'/0'/0/0"`)
  - The derivation path used for the Canton message. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
We encourage you to explore the Canton Signer by trying it out in our online [sample application](https://app.devicesdk.ledger-test.com/). Experience how it works and see its capabilities in action. Of course, you will need a Ledger device connected.
