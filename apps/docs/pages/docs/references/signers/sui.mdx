...
To initialise a Sui signer instance, you need a Ledger Device Management Kit instance and the ID of the session of the connected device. Use the `SignerSuiBuilder`:

```typescript
const signerSui = new SignerSuiBuilder({ dmk, sessionId }).build();
```
...
```typescript
const { observable, cancel } = signerSui.getAppConfig();
```
...
- `options`
  ...
  - `skipOpenApp`: An optional boolean indicating whether to skip opening the Sui app automatically (`true`) or not (`false`).
...
This method allows users to sign a Sui transaction.
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/0'/0'/0/0"`)
  - The derivation path used for the Sui transaction. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/0'/0'/0/0"`)
  - The derivation path used for the Sui message. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
