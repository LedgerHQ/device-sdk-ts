...
This module provides the implementation of the Ledger Icon signer of the Device Management Kit. It enables interaction with the Icon application on a Ledger device including:

- Retrieving the Icon address using a given derivation path
- Signing a Icon transaction
...
To install the `device-signer-kit-icon` package, run the following command:

```sh
npm install @ledgerhq/device-signer-kit-icon
```

## 🔹 Initialisation

To initialise a Icon signer instance, you need a Ledger Device Management Kit instance and the ID of the session of the connected device. Use the `SignerIconBuilder`:

```typescript
const signerIcon = new SignerIconBuilder({ dmk, sessionId }).build();
```

## 🔹 Use Cases

The `SignerIconBuilder.build()` method will return a `SignerIcon` instance that exposes 4 dedicated methods, each of which calls an independent use case. Each use case will return an object that contains an observable and a method called `cancel`.
...
```typescript
const { observable, cancel } = signerIcon.getAppConfig();
```
...
This method allows users to retrieve the Icon address based on a given `derivationPath`.

```typescript
const { observable, cancel } = signerIcon.getAddress(derivationPath, options);
```
...
- `derivationPath`
  ...
  - The derivation path used for the Icon address. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
- `options`
  ...
  - `skipOpenApp`: An optional boolean indicating whether to skip opening the Icon app automatically (`true`) or not (`false`).
...
This method allows users to sign a Icon transaction.

```typescript
const { observable, cancel } = signerIcon.signTransaction(
  derivationPath,
  transaction,
  options,
);
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/0'/0'/0/0"`)
  - The derivation path used for the Icon transaction. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
```typescript
const { observable, cancel } = signerIcon.signMessage(
  derivationPath,
  message,
);
```
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/0'/0'/0/0"`)
  - The derivation path used for the Icon message. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
We encourage you to explore the Icon Signer by trying it out in our online [sample application](https://app.devicesdk.ledger-test.com/). Experience how it works and see its capabilities in action. Of course, you will need a Ledger device connected.
