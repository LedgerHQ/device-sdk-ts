# Multiversx Signer Kit

This module provides the implementation of the Ledger Multiversx signer of the Device Management Kit. It enables interaction with the Multiversx application on a Ledger device including:

- Retrieving the Multiversx address using a given derivation path
- Signing a Multiversx transaction
...
To install the `device-signer-kit-multiversx` package, run the following command:

```sh
npm install @ledgerhq/device-signer-kit-multiversx
```

## 🔹 Initialisation

To initialise a Multiversx signer instance, you need a Ledger Device Management Kit instance and the ID of the session of the connected device. Use the `SignerMultiversxBuilder`:

```typescript
const signerMultiversx = new SignerMultiversxBuilder({ dmk, sessionId }).build();
```

## 🔹 Use Cases

The `SignerMultiversxBuilder.build()` method will return a `SignerMultiversx` instance that exposes 4 dedicated methods, each of which calls an independent use case. Each use case will return an object that contains an observable and a method called `cancel`.
...
```typescript
const { observable, cancel } = signerMultiversx.getAppConfig();
```
...
This method allows users to retrieve the Multiversx address based on a given `derivationPath`.

```typescript
const { observable, cancel } = signerMultiversx.getAddress(derivationPath, options);
```
...
- `derivationPath`
  ...
  - The derivation path used for the Multiversx address. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
- `options`
  ...
  - `skipOpenApp`: An optional boolean indicating whether to skip opening the Multiversx app automatically (`true`) or not (`false`).
...
This method allows users to sign a Multiversx transaction.

```typescript
const { observable, cancel } = signerMultiversx.signTransaction(
  derivationPath,
  transaction,
  options,
);
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/0'/0'/0/0"`)
  - The derivation path used for the Multiversx transaction. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
```typescript
const { observable, cancel } = signerMultiversx.signMessage(
  derivationPath,
  message,
);
```
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/0'/0'/0/0"`)
  - The derivation path used for the Multiversx message. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
We encourage you to explore the Multiversx Signer by trying it out in our online [sample application](https://app.devicesdk.ledger-test.com/). Experience how it works and see its capabilities in action. Of course, you will need a Ledger device connected.
