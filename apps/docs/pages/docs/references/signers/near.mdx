...
To initialise a Near signer instance, you need a Ledger Device Management Kit instance and the ID of the session of the connected device. Use the `SignerNearBuilder`:

```typescript
const signerNear = new SignerNearBuilder({ dmk, sessionId }).build();
```
...
```typescript
const { observable, cancel } = signerNear.getAppConfig();
```
...
This method allows users to retrieve the Near address based on a given `derivationPath`.

```typescript
const { observable, cancel } = signerNear.getAddress(derivationPath, options);
```
...
- `options`
  ...
  - `skipOpenApp`: An optional boolean indicating whether to skip opening the Near app automatically (`true`) or not (`false`).
...
This method allows users to sign a Near transaction.
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/0'/0'/0/0"`)
  - The derivation path used for the Near transaction. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
```typescript
const { observable, cancel } = signerNear.signMessage(
  derivationPath,
  message,
);
```
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/0'/0'/0/0"`)
  - The derivation path used for the Near message. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
