...
This module provides the implementation of the Ledger Kaspa signer of the Device Management Kit. It enables interaction with the Kaspa application on a Ledger device including:

- Retrieving the Kaspa address using a given derivation path
- Signing a Kaspa transaction
...
To install the `device-signer-kit-kaspa` package, run the following command:

```sh
npm install @ledgerhq/device-signer-kit-kaspa
```

## 🔹 Initialisation

To initialise a Kaspa signer instance, you need a Ledger Device Management Kit instance and the ID of the session of the connected device. Use the `SignerKaspaBuilder`:

```typescript
const signerKaspa = new SignerKaspaBuilder({ dmk, sessionId }).build();
```

## 🔹 Use Cases

The `SignerKaspaBuilder.build()` method will return a `SignerKaspa` instance that exposes 4 dedicated methods, each of which calls an independent use case. Each use case will return an object that contains an observable and a method called `cancel`.
...
```typescript
const { observable, cancel } = signerKaspa.getAppConfig();
```
...
This method allows users to retrieve the Kaspa address based on a given `derivationPath`.

```typescript
const { observable, cancel } = signerKaspa.getAddress(derivationPath, options);
```
...
- `derivationPath`
  ...
  - The derivation path used for the Kaspa address. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
- `options`
  ...
  - `skipOpenApp`: An optional boolean indicating whether to skip opening the Kaspa app automatically (`true`) or not (`false`).
...
This method allows users to sign a Kaspa transaction.

```typescript
const { observable, cancel } = signerKaspa.signTransaction(
  derivationPath,
  transaction,
  options,
);
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/0'/0'/0/0"`)
  - The derivation path used for the Kaspa transaction. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
```typescript
const { observable, cancel } = signerKaspa.signMessage(
  derivationPath,
  message,
);
```
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/0'/0'/0/0"`)
  - The derivation path used for the Kaspa message. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
We encourage you to explore the Kaspa Signer by trying it out in our online [sample application](https://app.devicesdk.ledger-test.com/). Experience how it works and see its capabilities in action. Of course, you will need a Ledger device connected.
