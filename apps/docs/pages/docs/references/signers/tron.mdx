...
This module provides the implementation of the Ledger Tron signer of the Device Management Kit. It enables interaction with the Tron application on a Ledger device including:
...
To initialise a Tron signer instance, you need a Ledger Device Management Kit instance and the ID of the session of the connected device. Use the `SignerTronBuilder`:

```typescript
const signerTron = new SignerTronBuilder({ dmk, sessionId }).build();
```

## 🔹 Use Cases

The `SignerTronBuilder.build()` method will return a `SignerTron` instance that exposes 4 dedicated methods, each of which calls an independent use case. Each use case will return an object that contains an observable and a method called `cancel`.
...
```typescript
const { observable, cancel } = signerTron.getAppConfig();
```
...
```typescript
const { observable, cancel } = signerTron.getAddress(derivationPath, options);
```
...
```typescript
const { observable, cancel } = signerTron.signTransaction(
  derivationPath,
  transaction,
  options,
);
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/0'/0'/0/0"`)
  - The derivation path used for the Tron transaction. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
```typescript
const { observable, cancel } = signerTron.signMessage(
  derivationPath,
  message,
);
```
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/0'/0'/0/0"`)
  - The derivation path used for the Tron message. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
