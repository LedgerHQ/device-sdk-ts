...
- Retrieving the Hedera address using a given derivation path
...
The `SignerHederaBuilder.build()` method will return a `SignerHedera` instance that exposes 4 dedicated methods, each of which calls an independent use case. Each use case will return an object that contains an observable and a method called `cancel`.
...
```typescript
const { observable, cancel } = signerHedera.getAppConfig();
```
...
This method allows users to retrieve the Hedera address based on a given `derivationPath`.
...
- `derivationPath`
  ...
  - The derivation path used for the Hedera address. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/0'/0'/0/0"`)
  - The derivation path used for the Hedera transaction. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
...
```typescript
const { observable, cancel } = signerHedera.signMessage(
  derivationPath,
  message,
);
```
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"m/44'/0'/0'/0/0"`)
  - The derivation path used for the Hedera message. See [here](https://www.ledger.com/blog/understanding-crypto-addresses-and-derivation-paths) for more information.
