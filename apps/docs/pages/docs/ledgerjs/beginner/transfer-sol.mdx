...
Before starting, make sure you have gone through the [prerequisites](../info#prerequisites).

### Install the Solana Embedded App

First you need to install the Solana Embedded App from Ledger Wallet as shown in the [prerequisites](../info#prerequisites) for Ethereum.
...
Put the logo in the assets folder and name it `ledger-logo`.
...
![Choice of signer](/device-interaction/tutorial-1-solana-wallet.png) (formerly 'wallet')
_Fig. 6: Choice of signer (formerly 'hardware wallet')_
...
The Solana Devnet is not visible on Ledger Wallet, you can then check the transaction passed on [the Solana Explorer](https://explorer.solana.com/?cluster=devnet).

#### Option n°1 - Solana Faucet

Go to the [Solana Faucet](https://solfaucet.com/) website put your signer Public Key (displayed on the web app) on the input and click on "Devnet"
