...
> **Warning:** Starting in September 2025, the Ethereum app will no longer support `signEIP712HashedMessage` due to a breaking change in EIP-712 signing. Any code that calls this method must migrate to `signTypedData` before that date. All other `hw-app-eth` methods will also stop working because `hw-app-eth` itself is deprecated.
...
| --- | --- | --- |
| **EIP-712 support** | `signEIP712HashedMessage` deprecated in September 2025 | `signTypedData` (full structured-data signing) |
...
This is the migration required before September 2025.
...
### `signEIP712HashedMessage`: no direct equivalent

`signEIP712HashedMessage` accepted a pre-computed domain separator hash and message hash (blind signing of hashed EIP-712 data). The Ethereum app is removing support for this in September 2025.

**You must migrate to `signTypedData`.** Pass the full structured data instead of the hashes:

```typescript
// Before: you computed hashes yourself and passed them
// await eth.signEIP712HashedMessage(path, domainSeparatorHex, hashStructMessageHex);

// After: pass the original TypedData (the app computes the hashes itself)
...
If you do not have access to the original structured data (only the hashes), you cannot use `signTypedData`. In that case, you need to modify your application to preserve the structured data before hashing.
...
| `signEIP712HashedMessage(path, domainHex, msgHex)` | **No equivalent** (migrate to `signTypedData`) | **Required before September 2025** |
...
| `signEIP712HashedMessage` stopped working | Migrate to `signTypedData`; see [the section above](#signeip712hashedmessage-no-direct-equivalent). |
