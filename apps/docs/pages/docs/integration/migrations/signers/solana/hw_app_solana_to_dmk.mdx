...
// address is a Buffer
const base58Address = address.toString("hex"); // or use bs58
```
...
observable.subscribe({
  next: (state) => {
    if (state.status === DeviceActionStatus.Completed) {
      const base58Address = state.output.publicKey; // already base58
    }
  },
});
```
...
- The address is returned as a **base58 string**, not a `Buffer`.
...
const { publicKey } = await toPromise(signer.getAddress("44'/501'/0'"));
