...
- Retrieving a unified shielded address (Orchard receiver) using a given transparent derivation path
...
4. [Use Cases](#-use-cases)
   ...
   - [Get Shielded Address](#use-case-7-get-shielded-address)
...
### Use Case 7: Get Shielded Address

This method allows users to retrieve a unified shielded address (single Orchard receiver) based on a transparent derivation path. The device uses the transparent path for account matching and on-device display; the returned unified address begins with `u1`.
...
- `derivationPath`
  ...
  - **Required**
  - **Type:** `string` (e.g., `"44'/133'/0'/0/0"`)
  - Transparent derivation path (BIP-44). The Orchard account path (`32'/coin/account'`) is derived from it automatically.
