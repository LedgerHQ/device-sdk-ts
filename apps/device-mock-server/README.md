...
### App versions come from the Manager API

An app's version is tied to a firmware version — Speculos resolves an ELF at
`/apps/{device}/{firmware}/{App}/app_{version}.elf`, so a version paired with the
wrong firmware simply is not there and Open App fails. Rather than ship a fixed
list that goes stale, the server exposes `GET /catalog/apps?device_type=&firmware_version=`,
which asks the Manager API (`/v2/apps/by-target`) for the apps that exist for the
target id that model reports on that firmware. Answers are cached per
model/firmware pair. The UI's app picker reads this, so a device can only be
given app versions that exist.

`GET /catalog/firmware?device_type=&firmware_version=` answers the prior
question — was this OS version ever released for this model? It resolves the
target id to a Manager API device version, then looks the firmware up by name
(a `404` there is the answer "no such version", not a failure). The UI checks
this as the firmware field is typed, because an OS that never shipped is the
usual reason an app list comes back empty.
