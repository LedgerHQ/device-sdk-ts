...
interface ConsolePanelProps {
  ...
  /** Called after an APDU that can change the device (onboarding, open app). */
      ...
      <div className="flex flex-col gap-8">
        ...
        <div className="flex flex-wrap gap-6">
          <Button
            appearance="gray"
            size="sm"
            onClick={() => void send(GET_OS_VERSION_APDU)}
          >
            Get OS version
          </Button>
          <Button
            appearance="gray"
            size="sm"
            onClick={() => void send(GET_APP_AND_VERSION_APDU)}
          >
            Get running app
          </Button>
          <Button
            appearance="gray"
            size="sm"
            onClick={() => void send(CLOSE_APP_APDU)}
          >
            Close app
          </Button>
          {(device.apps ?? [])
