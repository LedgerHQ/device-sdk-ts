/** Helpers for the APDU console: building the common commands and reading SWs. */
...
export const openAppApdu = (appName: string): string =>
  `e0d800${toHexByte(appName.length)}${asciiToHex(appName)}`;
...
/** Well-known status words, so a raw reply reads as something meaningful. */
...
/** Split an APDU response into its data and a label for its status word. */
