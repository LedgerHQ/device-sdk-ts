...
`src/api/client.ts` is the only place that knows about the mock server's HTTP API. It is a thin `fetch` wrapper — no DMK dependency — that types its payloads with the contract from [`@ledgerhq/device-mockserver-client`](../../packages/mockserver-client) so the UI and the server cannot drift apart, and raises `MockServerError` carrying the status code (a 401 means "that token is gone", not "bad request").

The one exception is the app and OS-version lookup in `src/api/managerApi.ts`, which calls Ledger's Manager API directly. It carries no session and is read-only, so proxying it would mean adding endpoints to the mock server's deployed contract for the sake of one dialog; the API allows every origin.
