...
export class SpeculosServiceController implements ServiceController {
  ...
  async start(): Promise<void> {
    ...
    this.warnIfLatestImageIsStale(dockerImage).catch((error) => {
      this.logger.debug(
        `Staleness check failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    });
  ...
  private async getRemoteImageVersion(
    ...
    try {
      ...
      const platformData = Object.values(imageData)[0];
      return (
