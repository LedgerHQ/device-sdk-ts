...
export class EtherscanService {
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        if (error.code === 'ECONNABORTED') {
          throw new Error(`Timeout fetching transactions for ${contractAddress}`);
        }
        ...
        throw new Error(`HTTP error fetching transactions for ${contractAddress}: ${error.message}`);
      }
      throw new Error(`Error fetching transactions for ${contractAddress}: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async getTransactionByHash(txHash: string): Promise<EtherscanTransaction> {
    try {
      ...
      if (axios.isAxiosError(error)) {
        throw new Error(`HTTP error fetching transaction ${txHash}: ${error.message}`);
      }
  ...
  async isAddressContract(address: string): Promise<boolean> {
    try {
      ...
      const response = await axios.get<EtherscanCodeResponse>(this.baseUrl, {
        params: {
          module: 'proxy',
          action: 'eth_getCode',
          address: address,
      ...
      if (response.data.status === '1') {
        ...
        console.log(`✅ ${address} is ${isContract ? 'a contract' : 'not a contract'}`);
        return isContract;
      } else {
        throw new Error(`Etherscan API error: ${response.data.message}`);
      }
      ...
      if (axios.isAxiosError(error)) {
        if (error.code === 'ECONNABORTED') {
          throw new Error(`Timeout checking contract status for ${address}`);
        }
        if (error.response?.status === 403) {
          throw new Error(`Invalid API key for Etherscan`);
        }
        ...
        throw new Error(`HTTP error checking contract status for ${address}: ${error.message}`);
      }
      throw new Error(`Error checking contract status for ${address}: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
} 